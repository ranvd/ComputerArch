...
asm("li sp, 0x7ffffff0");  // SP set to 1 MB
asm("jal main");         // call main
asm("li a7, 2");        // prepare ecall exit
...
int mySin(int x) {
    int r = 0;
    for (int n = 0; n < 5;
         n++) {  // You can adjust the number of terms for accuracy
...
int main() {
    int sinev = mySin(0x3f490fdb);  // Calculate sine
    return sinev;
}
